...
import { headerNav, navElements, navInnerWrap, navAuctionText, popUpOverlay, hamburgerIcon, navMenu } from '../js-files/utilities/globals-vars.js'
...
export function initAuctionPage() {
    ...
    // hamburger menu visible only for artists
    ...
    function fillAuctionItemContainer() {
        ...
        if (auctioningItemIndex !== -1) {
            ...
            initTimer(whenDone)
             ...
             yourBidsList.length = 0
            othersBidsList.length = 0
            highestBid = 0
    ...
    function updateBidsFromLocalStorage() {
        ...
        yourBids.innerHTML = yourBidsList.map(bid => `<li>$${bid}<img src="images/assets/bid-icon-left-1.png" alt="bid-icon"></li>`).join('')
        otherBids.innerHTML = othersBidsList.map(bid => `<li><img src="images/assets/bid-icon-right-1.png" alt="bid-icon">$${bid}</li>`).join('')
    }
    ...
    function validateBiddingInput() {
        ...
        if (isNaN(biddingInputValue)) {
            ...
            setBiddingError(biddingInput, 'Bid cannot be lower or equal to the initial price!')
        } else if (allBids.length !== 0 && biddingInputValue < highestBid + 50) {
            setBiddingError(biddingInput, 'Require bids $50 higher than current highest bid.')
        } else {
            setBiddingSuccess(biddingInput, 'Good luck!')
        }

        // second condition/else if is made as it is, so you can bet a little above
        // the initial price on first bid. E.g. initial price is 10, you can bet
        // 11 instead of 60, after that you have to bid 50 more
    }
    ...
    biddingForm.addEventListener('submit', function (ev) {
        ...
        const allGood =
            ...
            (allBids.length === 0 || biddingInput.value >= highestBid + 50)

        // last condition is made so you can bet less than 50(if initial price is
        // below that), and a bug that would happen in this scenario since
        // highestBid is taken either from local storage or set to 0
        ...
        if (!bidBtn.hasAttribute('disabled')) {
            ...
            if (allGood) {
                ...
                console.log('yourBids:' + yourBidsList)
                console.log('otherBids:' + othersBidsList)
            } else {
                return
            }
        }
    ...
    function initTimer(whenDone, startingTime = initialAuctionTime) {
        ...
        const storedTimestamp = +(localStorage.getItem("startingTimeTimestamp")) || Date.now()
        ...
        if (elapsedTime === 0) {
            localStorage.setItem("startingTimeTimestamp", currentTime.toString())
        }
    ...
    function deleteLocalStorageListsAndTime() {
        ...
        localStorage.removeItem('highestBid')
        localStorage.removeItem('time')
        localStorage.removeItem('startingTimeTimestamp')
    }
    ...
    function clearAuction() {
        ...
        // handle scenario where user bid is higher or API does not bid after first bid

        const youWon = yourBidsList.length > 0 && (othersBidsList.length === 0 || yourBidsList[yourBidsList.length - 1] > othersBidsList[othersBidsList.length - 1])

        // handle scenario where user bid is lower than API bid
        ...
        if (youWon) {
            ...
            // custom message if you're logged in as an artist, user wins
            ...
            // custom message if you're logged in as an artist, API wins
